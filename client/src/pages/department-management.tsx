...
import { insertDepartmentSchema, type Department, type InsertDepartment, type User } from "@shared/schema";
...
export default function DepartmentManagement() {
  ...
  const getDepartmentEmployeeCount = (departmentId: number) => {
    return users?.filter(u => u.departmentId === departmentId && u.active === "true").length || 0;
  };
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {editingDepartment ? "Atualize as informações do departamento" : "Adicione um novo departamento"}
                  </DialogDescription>
                </DialogHeader>
                          ...
                          <FormItem>
                            ...
                            <Select onValueChange={(value) => field.onChange(Number(value))} value={field.value?.toString()}>
                              ...
                              <SelectContent>
                                ...
                                {users?.filter(u => u.active === "true").map((user) => (
            ...
            {filteredDepartments.map((department) => (
              <Card key={department.id} className="hover:shadow-lg transition-shadow">
                <CardHeader className="pb-4">
                  ...
                  <CardTitle className="text-lg text-gray-900 dark:text-gray-100">{department.name}</CardTitle>
                ...
                <CardContent className="space-y-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      <strong>Gerente:</strong> {getManagerName(department.managerId)}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      <strong>Funcionários:</strong> {getDepartmentEmployeeCount(department.id)}
                    </span>
                  </div>
                  ...
                  {department.budget && (
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-gray-700 dark:text-gray-300">
                        <strong>Orçamento:</strong> R$ {parseFloat(department.budget).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                      </span>
                    </div>
                  )}

                  <div className="flex items-center justify-between pt-2">
                    <Badge variant={department.active === "true" ? "default" : "secondary"}>
          ...
          {filteredDepartments.length === 0 && (
            <Card>
              <CardContent className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Nenhum departamento encontrado</h3>
                <p className="text-gray-600 dark:text-gray-300">
                  {searchTerm ? "Tente ajustar sua busca" : "Comece criando um novo departamento"}
                </p>
              </CardContent>
            </Card>
          )}
